const toGarage = document.querySelector('.options_butt-garage');
const toWinners = document.querySelector('.options_butt-winners');
const winners = document.querySelector('.winners');
let winnersList = document.querySelector('.winners_table');
const amountWinners= document.querySelector('.winners_amount');
const winnersPage = document.querySelector('.winners_page');
const prevWinnersPage = document.querySelector('.page-toggle-winners_prev');
const nextWinnersPage = document.querySelector('.page-toggle-winners_next');
let pageWinners = localStorage.getItem('pageWinners-saved') || 1;
let maxPageWinners;
let typeWinsSort = true;   // есои true то по убыванию
let typeTimesSort = true;
...
toGarage.addEventListener('click', () => {
    ...
    localStorage['status-format'] = 'true';
...
toWinners.addEventListener('click', () => {
    ...
    localStorage['status-format'] = 'false';
...
function fillWinners() {
    ...
    `<tr>
        ...
        <td>Car</td>
        <td>Name</td>
        <td class="wins">Wins <span class="arrow-wins inactive">&#10148;</span></td>
        <td class="best-time">Best time (seconds)   <span class="arrow-times inactive">  &#10148;</span></td>
    </tr> `;

    winnersList.innerHTML = currentWinnersList;

    sendRequest('GET', urlWinners)
    .then(data => {
        maxPageWinners = data.length / 10;
        amountWinners.textContent = `Winners (${data.length})`;
        winnersPage.textContent = `Page #${pageWinners}`;

        let i = 0;
        if(pageWinners > 1) i = (10 * (pageWinners - 1));

        for(let j = 0; j <= 10 - (maxPageWinners * 10 - data.length); j++) {
            if(j > 9 || j > data.length - 1 - ((pageWinners - 1) * 10)) break

            sendRequest('GET', urlGarage, null, data[(pageWinners - 1) * 10 + j].id)
            .then(info => {
                currentWinnersList += ` 
                <tr class="winner">
                    <td class="number">SomeNumber</td>
                    <td>
                        <div class="winner_model" id="${info.id}">
                        <svg version="1.0" xmlns="http://www.w3.org/2000/svg"
                        width="120px" height="50px" viewBox="0 0 1250.000000 540.000000"
                        preserveAspectRatio="xMidYMid meet">
                        <metadata>
                        Created by potrace 1.15, written by Peter Selinger 2001-2017
                        </metadata>
                        <g transform="translate(0.000000,640.000000) scale(0.100000,-0.100000)"
                        fill="${info.color}" stroke="none">
                    ...
                    <td>${info.name}</td>
                    <td class="win-counter">${data[i].wins}</td>
                    <td class="time-counter">${data[i].time}</td>
                </tr>`;
            ...
            .then(() => {
                if(i == data.length || i == 10) {
                    ...
                    for(let i=1; i < 10 - (pageWinners * 10 - data.length) + 1; i++) {  
                        currentWinnersList = currentWinnersList.replace('SomeNumber', `${10 * pageWinners - (10 - i)}`)
                    }
                    winnersList.innerHTML = currentWinnersList;

                    // Сортировка по кол-ву побед
                    const wins = document.querySelector('.wins');
                    const arrowWins = document.querySelector('.arrow-wins');
                    const arrowTimes = document.querySelector('.arrow-times');

                    wins.addEventListener('click', () => {
                        const win = document.querySelectorAll('.win-counter');
                        const winnerCar = document.querySelectorAll('.winner');

                        arrowWins.classList.remove('inactive');
                        arrowTimes.classList.add('inactive');

                        if(typeWinsSort == true){
                            arrowWins.classList.add('down');
                            arrowWins.classList.remove('up');
                            
                        } else {
                            arrowWins.classList.add('up');
                            arrowWins.classList.remove('down');
                        }

                        let arrWinSort = [];
                        let arrWin = Array.from(win)

                        for(let elem of arrWin) {
                            let obj = {};
                            obj.wins = elem.textContent;
                            obj.index = arrWin.indexOf(elem);
                            arrWinSort.push(obj)
                        }

                        if(typeWinsSort) {
                            arrWinSort.sort((a, b) => b.wins > a.wins ? 1 : -1);
                            typeWinsSort = false;
                        } 
                        else {
                            arrWinSort.sort((a, b) => a.wins > b.wins ? 1 : -1);
                            typeWinsSort = true;
                        };

                        winnerCar.forEach((elem) => {
                            elem.remove()
                        });

                        for(let obj of arrWinSort) {
                            winnersList.insertAdjacentHTML('beforeend', winnerCar[obj.index].outerHTML)
                        }
                    ...
                    bestTime.addEventListener('click', () => {
                        const time = document.querySelectorAll('.time-counter');
                        const winnerCar = document.querySelectorAll('.winner');

                        arrowTimes.classList.remove('inactive');
                        arrowWins.classList.add('inactive');

                        if(typeTimesSort == true){
                            arrowTimes.classList.add('down');
                            arrowTimes.classList.remove('up');
                            
                        } else {
                            arrowTimes.classList.add('up');
                            arrowTimes.classList.remove('down');
                        }

                        let arrTimeSort = [];
                        let arrTime = Array.from(time)

                        for(let elem of arrTime) {
                            let obj = {};
                            obj.time = elem.textContent;
                            obj.index = arrTime.indexOf(elem);
                            arrTimeSort.push(obj)
                        }

                        if(typeTimesSort) {
                            arrTimeSort.sort((a, b) => b.time > a.time ? 1 : -1);
                            typeTimesSort = false;
                        } 
                        else {
                            arrTimeSort.sort((a, b) => a.wins > b.wins ? 1 : -1);
                            typeTimesSort = true;
                        };
...
nextWinnersPage.addEventListener('click', () => {
    ...
    localStorage.setItem('pageWinners-saved', ++pageWinners);
    if(pageWinners > Math.ceil(maxPageWinners)) localStorage.setItem('pageWinners-saved', --pageWinners);
    console.log(localStorage['pageWinners-saved'])
...
prevWinnersPage.addEventListener('click', () => {
    ...
    localStorage.setItem('pageWinners-saved', --pageWinners);
    if(pageWinners < 1) localStorage.setItem('pageWinners-saved', ++pageWinners);
    console.log(localStorage['pageWinners-saved'])
